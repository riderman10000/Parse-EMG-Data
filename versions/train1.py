...
sample_rate = 16000
win_size = 40 # 40 ms 
n_fft = int (win_size/1000* sample_rate)
hop_length = int(10 / 1000 * sample_rate)

# import librosa 
# feature = librosa.feature.melspectrogram(words[1], sr = sample_rate, n_fft = n_fft, hop_length = hop_length, window = 'hann')

# import librosa.display
# s_db = librosa.power_to_db(feature, ref = np.max)
# print(s_db)
# print(s_db.shape)
...
feature = []
for word in words:
	spectrogram = librosa.feature.melspectrogram(word, sr = sample_rate, n_fft = n_fft, hop_length = hop_length, window = 'hann')
	feature.append(spectrogram)
...
clf = Sequential()
clf.add(Convolution2D(32,3,3, input_shape = (feature.shape[1], feature.shape[2],feature.shape[3]), activation = 'relu' ))
